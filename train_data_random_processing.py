...
for i, uid in enumerate(train_data['user'].unique()):
    percent_len = math.floor(len(train_data[train_data['user']==uid])*MOVIE_PERCENT/100)
    uid_perm = np.random.permutation(percent_len)
    if i == 0 :
        train_processing = train_data[train_data['user']==uid].iloc[uid_perm]
    else:
        train_processing = train_processing.append(train_data[train_data['user']==uid].iloc[uid_perm])

print("====================== 결과 ======================")
print(len(train_processing))
print(len(train_data))

pro_dir = os.path.join(DATA_DIR, f'pro_sg_random_{MOVIE_PERCENT}')
...
vad_plays = train_processing.loc[train_processing['user'].isin(vd_users)]
vad_plays = vad_plays.loc[vad_plays['item'].isin(unique_sid)]
vad_plays_tr, vad_plays_te = split_train_test_proportion(vad_plays)

test_plays = train_processing.loc[train_processing['user'].isin(te_users)]
